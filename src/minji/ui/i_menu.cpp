...
namespace
{
    ...
    void practice_menu()
    {
        ...
        switch (opt) {
            case practice::practice_opt::numbered: {
                ...
                if (!utils::parse_input(
                        count, [](int count) { return count < 0; })
                    || !count)
                    return;
                break;
            }
        ...
        std::optional<std::unique_ptr<minji::answer>>& opt_back)
    {
        ...
        switch (*type) {
            ...
            case minji::answer_type::multiple_choice: {
                unsigned ans_count;
                char sure;
                auto invalid_confirm
                    = [](char c) { return c != 'y' && c != 'n' && c != 'c'; };
                do {
                    std::cout << "[input] total number of choices: "
                                 "(enter 0 or nothing to cancel)\n";
                    if (!utils::parse_input(ans_count) || !ans_count)
                        return;
    ...
    void list_card_menu()
    {
        ...
        auto invalid_in = [size = current_deck.lock()->size()](
                              size_t idx) { return idx > size; };
        do {
            if (idx) {
                utils::clear_screen();
                current_deck.lock()->show_card(
                    std::cout, idx - 1, minji::answer::reveal::yes);
                std::cout << '\n';
            }
            std::cout << "[list] cards:\n";
            list_card();
    ...
    void change_card()
    {
        ...
        list_card();
        unsigned idx;
        auto invalid_in = [limit = current_deck.lock()->size()](
                              size_t idx) { return idx > limit; };
    ...
    void add_deck(minji::collection& clt)
    {
        ...
        auto same_name = [&clt](const std::string& name) {
            return std::find_if(begin(clt), end(clt),
                       [&name](
                           const auto& deck) { return deck->name() == name; })
                != clt.end();
        };
        do {
            if (!name.empty())
    ...
    void change_deck(minji::collection& clt)
    {
        ...
        auto invalid_in
            = [limit = clt.size() + 2](size_t in) { return in > limit; };
        size_t idx;
        if (!utils::parse_input(idx, invalid_in) || !idx--)
            return;
    ...
    void remove_deck(minji::collection& clt)
    {
        ...
        unsigned idx;
        auto invalid_in
            = [size = clt.size()](unsigned idx) { return --idx > size; };
