...
namespace minji
{
    ...
    std::string& card::description() { return description_; }
    ...
    answer& card::back() { return *back_; }

    const answer& card::back() const { return const_cast<card*>(this)->back(); }
    ...
    {
        return this == &c
            || (this->front_ == c.front_ && this->back_ == c.back_
...
namespace minji
{
    card_builder::card_builder(std::string deck_name)
    ...
    {
        return card(std::move(description_), std::move(front_), std::move(ans_),
            std::move(deck_name_));
    }
}
