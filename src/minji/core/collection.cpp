...
namespace minji
{
    ...
    void collection::clear() { decks_.clear(); }

    size_t collection::size() const { return decks_.size(); }

    bool collection::empty() const { return !this->size(); }
    ...
    collection::value_type& collection::back() { return decks_.back(); }
    ...
    std::vector<collection::value_type>::const_iterator
