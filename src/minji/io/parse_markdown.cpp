...
namespace
{
    struct card_component_mark {
        ...
        bool all_checked() { return description && front && back; }

        void clear() { description = front = back = false; }
...
namespace io
{
    std::shared_ptr<minji::deck> parse_markdown(
        const std::filesystem::path& p, const std::string& deck_name)
    {
